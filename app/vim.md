...
例えば、行の先頭が `A` 以外の文字で始まる行をマッチさせるには、以下の正規表現を使用します。
...
これにより、「行の先頭が `A` ではない文字で始まる行」にマッチします。

### 応用
- もし「`A` 以外の文字列」をマッチさせたい場合は、`[^A]` の部分を他の否定したい文字に変更することで対応できます。例えば、「`A` と `B` 以外の文字」をマッチさせたい場合は、`[^AB]` を使います。

## 文字列の否定
複数の文字列や単語を否定したい場合は、次のようにパターンを工夫します。

### 方法 1: `\v` を使ったパターンの書き方
Vim の正規表現モードで「パターン」自体を否定する方法を使います。`\v` を使うことで、簡潔な正規表現を使用できます。

例えば、「行の先頭が 'AB' または 'CD' ではない行」をマッチさせたい場合：

```vim
\v^(?!AB|CD)
```

- `\v` : 「非常に魔法的な」正規表現モードを有効にします。これにより、通常より短い形式で正規表現を書くことができます。
- `(?!...)` : 先読みの否定。ここでは "AB" または "CD" で始まる行を否定しています。
...
ノーマルモード中に `qa` を押すことで操作を記録できる。

`q` の後に任意のアルファベット 1 文字を押すと、その文字のレジスタに記録したキー操作が保存される。

例えば、 `qa` ならば `a` レジスタに記録される。 以降は便宜上、レジスタは全て `a` を使って説明する。

記録中に再度 `q` を押すと操作の記録を終了する。

### マクロの確認

`q` で記録した操作は、ヤンクの際に使用されるレジスタに保存される。

記録内容をテキストとして出力するには、レジスタからテキストをペーストする。

    "ap

また、 `<C-r>a` でも同じく取り出せるが、環境依存なのか結果に差異が出た。

改行文字等に関して出力結果に違いが出て、後者の方法では正しく編集してレジスタに戻せなかった。

### 編集したマクロを入力する

編集した内容をレジスタに戻せば、結果的にマクロを編集したことになる。

レジスタに戻したい内容をビジュアルモードで選択してコマンド
